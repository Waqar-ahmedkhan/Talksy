...
export const authenticateToken = async (req, res, next) => {
  try {
    ...
    console.log(`[authenticateToken] Token decoded: phone=${decoded.phone}, iat=${decoded.iat}, exp=${decoded.exp}`);
    ...
    console.log(`[authenticateToken] User attached: userId=${user._id}, phone=${decoded.phone}`);
    next();
  } catch (err) {
...
const formatProfile = (profile, user, customName = null) => {
  ...
  console.log(`[formatProfile] Formatted profile: phone=${phone}, displayName=${displayName}, customName=${customName}`);
  return formatted;
};
...
const formatChat = (chat) => {
  ...
  const formatted = {
    ...
    type: chat?.type || "text",
    content:
      chat?.content?.substring(0, 50) + (chat?.content?.length > 50 ? "..." : "") || "",
  ...
  console.log(`[formatChat] Formatted chat: id=${chatId}, type=${formatted.type}, content=${formatted.content}`);
  return formatted;
};
...
export const getMyProfile = async (req, res) => {
  try {
    console.log(`[getMyProfile] Fetching profile: phone=${req.user.phone}, userId=${req.user._id}`);
    const profile = await Profile.findOne({ phone: req.user.phone });
    if (!profile) {
      console.error(`[getMyProfile] Profile not found: phone=${req.user.phone}`);
      return res.status(404).json({ success: false, error: "Profile not found" });
    }

    const user = await User.findOne({ phone: req.user.phone });
    console.log(`[getMyProfile] User ${user ? "found" : "not found"}: phone=${req.user.phone}`);

    const contact = await Contact.findOne({ userId: req.user._id, phone: req.user.phone }).select("customName");
    const customName = contact?.customName || null;
    console.log(`[getMyProfile] Custom name: phone=${req.user.phone}, customName=${customName}`);
...
export const getPublicProfiles = async (req, res) => {
  try {
    console.log(`[getPublicProfiles] Processing request: query=${JSON.stringify(req.query)}, userId=${req.user._id}`);
    ...
    const response = {
      ...
      page,
      limit,
      profiles: publicProfiles.map((profile) => formatProfile(profile, userMap.get(profile.phone), contactMap.get(profile.phone))),
    };
    console.log(`[getPublicProfiles] Response ready: profiles=${response.profiles.length}`);
...
export const getProfilesFromContacts = async (req, res) => {
  try {
    console.log(`[getProfilesFromContacts] Processing request: body=${JSON.stringify(req.body)}, userId=${req.user._id}`);
    const { contacts } = req.body;
    const userId = req.user._id;

    if (!Array.isArray(contacts) || contacts.length === 0) {
      console.error("[getProfilesFromContacts] Invalid or empty contacts array");
      return res.status(400).json({ success: false, error: "Contacts array is required" });
    }
    console.log(`[getProfilesFromContacts] Validated contacts: count=${contacts.length}`);
    ...
    if (typeof contacts[0] === "string") {
      ...
      const userContacts = await Contact.find({ userId, phone: { $in: phoneNumbers } }).select("phone customName");
      console.log(`[getProfilesFromContacts] Found ${userContacts.length} contacts`);
      userContacts.forEach((contact) => {
        const normalizedPhone = normalizePhoneNumber(contact.phone);
        console.log(`[getProfilesFromContacts] Mapping contact: phone=${normalizedPhone}, customName=${contact.customName || null}`);
      ...
      for (const contact of contacts) {
        ...
        contactMap.set(normalizePhoneNumber(contact.phone), contact.customName || null);
      }
      const userContacts = await Contact.find({ userId, phone: { $in: phoneNumbers } }).select("phone customName");
      ...
      userContacts.forEach((contact) => {
        ...
        if (!contactMap.has(normalizedPhone)) {
          console.log(`[getProfilesFromContacts] Merging contact: phone=${normalizedPhone}, customName=${contact.customName || null}`);
          contactMap.set(normalizedPhone, contact.customName || null);
        }
      });
    }

    const matchedProfiles = await Profile.find({ phone: { $in: phoneNumbers } }).select(
    ...
    const response = {
      success: true,
      profiles: matchedProfiles.map((profile) => formatProfile(profile, userMap.get(profile.phone), contactMap.get(profile.phone))),
    };
    console.log(`[getProfilesFromContacts] Response ready: profiles=${response.profiles.length}`);
    return res.json(response);
  } catch (err) {
...
export const getProfileWithChat = async (req, res) => {
  try {
    console.log(`[getProfileWithChat] Processing request: params=${JSON.stringify(req.params)}, userId=${req.user._id}`);
    ...
    if (!targetProfile) {
      console.error(`[getProfileWithChat] Profile not found: phone=${targetPhone}`);
    ...
    console.log(`[getProfileWithChat] Target user ${targetUser ? "found" : "not found"}: phone=${targetPhone}`);

    const contact = await Contact.findOne({ userId: req.user._id, phone: targetPhone }).select("customName");
    const customName = contact?.customName || null;
    console.log(`[getProfileWithChat] Custom name: phone=${targetPhone}, customName=${customName}`);
    ...
    console.log(`[getProfileWithChat] Response ready: chats=${response.chatHistory.length}`);
    return res.json(response);
  } catch (err) {
...
export const upsertContact = async (req, res) => {
  try {
    console.log(`[upsertContact] Processing request: body=${JSON.stringify(req.body)}, userId=${req.user._id}`);
    const { phone, customName } = req.body;
    const userId = req.user._id;

    if (!phone || typeof phone !== "string" || !phone.trim()) {
      console.error("[upsertContact] Invalid phone number: must be a non-empty string");
      return res.status(400).json({ success: false, error: "Valid phone number is required" });
    }

    if (!customName || typeof customName !== "string" || !customName.trim()) {
      console.error("[upsertContact] Invalid custom name: must be a non-empty string");
      return res.status(400).json({ success: false, error: "Valid custom name is required" });
    }

    const normalizedPhone = normalizePhoneNumber(phone);
    const phoneRegex = /^\+?[1-9]\d{1,14}$/;
    if (!phoneRegex.test(normalizedPhone)) {
      console.error(`[upsertContact] Invalid phone number format: ${normalizedPhone}`);
    ...
    console.log(`[upsertContact] Contact saved: phone=${normalizedPhone}, customName=${contact.customName}, contactId=${contact._id}`);
...
export const getChatList = async (req, res) => {
  try {
    console.log(`[getChatList] Processing request: query=${JSON.stringify(req.query)}, userId=${req.user._id}, phone=${req.user.phone}`);
    ...
    if (!userId || !myPhone) {
      ...
      return res.status(401).json({ success: false, error: "Unauthorized: Missing user ID or phone" });
    }

    if (page < 1 || limit < 1 || limit > 100) {
      console.error(`[getChatList] Invalid pagination: page=${page}, limit=${limit}`);
    ...
    console.log(`[getChatList] Extracted ${phoneNumbers.length} unique phone numbers`);
    ...
    console.log(`[getChatList] Fetching contacts: userId=${userId}, phones=${phoneNumbers.length}`);
    ...
    contacts.forEach((contact) => {
      const normalizedPhone = normalizePhoneNumber(contact.phone);
      console.log(`[getChatList] Mapping contact: phone=${normalizedPhone}, customName=${contact.customName || null}`);
      contactMap.set(normalizedPhone, contact.customName || null);
    });
    ...
    for (const chat of chats) {
      if (!chat.senderId || !chat.receiverId) {
        console.warn(`[getChatList] Skipping chat ${chat._id}: missing senderId or receiverId`);
        continue;
      }
      ...
      if (!chatMap.has(otherProfileId)) {
        ...
        console.log(`[getChatList] Profile: phone=${otherPhone}, displayName=${displayName}, customName=${customName}`);
        ...
        if (new Date(chat.createdAt) > new Date(existing.latestMessage.createdAt)) {
          console.log(`[getChatList] Updating latest message: profileId=${otherProfileId}, chatId=${chat._id}`);
    ...
    console.log(`[getChatList] Prepared ${chatList.length} chats: page=${page}, limit=${limit}`);
    ...
    console.log(`[getChatList] Response ready: total=${response.total}, chats=${formattedChatList.length}`);
    return res.json(response);
  } catch (err) {
...
export const deleteUserChat = async (req, res) => {
  try {
    console.log(`[deleteUserChat] Processing request: body=${JSON.stringify(req.body)}, userId=${req.user._id}`);
    ...
    if (!phoneRegex.test(normalizedTargetPhone)) {
      console.error(`[deleteUserChat] Invalid phone number format: ${normalizedTargetPhone}`);
      return res.status(400).json({ success: false, error: "Invalid phone number format" });
    }
    ...
    if (!targetProfile) {
      console.error(`[deleteUserChat] Profile not found: phone=${normalizedTargetPhone}`);
      return res.status(404).json({ success: false, error: "Target profile not found" });
    }
    ...
    console.log(`[deleteUserChat] Soft-deleted ${updateResult.modifiedCount} chats: userId=${userId}, targetPhone=${normalizedTargetPhone}`);

    if (updateResult.modifiedCount === 0) {
      console.log(`[deleteUserChat] No chats found to delete: userId=${userId}, targetPhone=${normalizedTargetPhone}`);
      return res.json({ success: true, message: "No chats found to delete", modifiedCount: 0 });
    }
