...
import fs from "fs";
import { fileTypeFromStream } from "file-type"; // New import for MIME detection
...
// Helper to detect true MIME type from file content
const detectMimeType = async (filePath, originalMime) => {
  try {
    const stream = fs.createReadStream(filePath);
    const fileTypeResult = await fileTypeFromStream(stream);
    return fileTypeResult ? fileTypeResult.mime : originalMime;
  } catch (err) {
    console.error("MIME detection error:", err);
    return originalMime; // Fallback to original if detection fails
  }
};
...
router.post("/", upload.single("file"), handleMulterError, async (req, res) => {
  try {
    ...
    // Detect true MIME type based on content
    req.file.mimetype = await detectMimeType(req.file.path, req.file.mimetype);
    ...
    if (req.file) {
      try {
        ...
        console.error(
          `Failed to delete temp file ${req.file.path}:`,
          cleanupErr
        );
      }
    }
  ...
  async (req, res) => {
    try {
      ...
      for (const file of files) {
        // Detect true MIME type based on content
        file.mimetype = await detectMimeType(file.path, file.mimetype);
