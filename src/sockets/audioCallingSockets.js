...
export const initAudioSocket = (server) => {
  ...
  io.on("connection", (socket) => {
    console.log("User connected to audio socket:", socket.id, "at", new Date().toLocaleString("en-PK", { timeZone: "Asia/Karachi" }));
    ...
    socket.on("join", async (userId) => {
      ...
      console.log(`User ${userIdStr} joined audio socket and marked online`);
    ...
    socket.on("call_user", async ({ callerId, calleeId, offer }) => {
      try {
        ...
        console.log(`Call initiated: ${caller} → ${callee}`);
      } catch (err) {
    ...
    socket.on("accept_call", ({ callerId, calleeId, answer }) => {
      ...
      console.log(`Call accepted: ${caller} ↔ ${callee}`);
    });
    ...
    socket.on("disconnect", async () => {
      ...
      console.log("User disconnected from audio socket:", disconnectedUserId);
