...
export const initGroupSocket = (server) => {
  ...
  io.on("connection", (socket) => {
    console.log(
      `[GROUP_SOCKET] User connected: socketId=${socket.id
    ...
    // socket.on("create_group", async (data, callback) => {
    //   console.log(
    //     `[CREATE_GROUP] Attempting: userId=${
    //       socket.userId
    //     }, data=${JSON.stringify(data)}`
    //   );
    ...
    //     // ✅ FIXED URL validation - handles query parameters
    ...
    //       console.error(`[CREATE_GROUP_ERROR] Members must be an array`);
    //       return callback({
    //         success: false,
    ...
    //       console.warn(`[CREATE_GROUP_WARN] Filtered out invalid member IDs`);
    //     }
    ...
    //       console.error(`[CREATE_GROUP_ERROR] One or more members not found`);
    //       return callback({
    //         success: false,
    //         message: "One or more members not found",
    //       });
    ...
    //     await group.save();
    //     console.log(
    //       `[CREATE_GROUP] Group created: groupId=${group._id}, name=${group.name}`
    //     );

    //     const groupRoom = `group_${group._id}`;
    //     socket.join(groupRoom);
    //     console.log(`[CREATE_GROUP] Creator joined room: groupId=${group._id}`);
    ...
    //       if (memberSocketId) {
    //         io.to(memberSocketId).emit("group_created", { group });
    //         console.log(`[CREATE_GROUP] Notified member: memberId=${memberId}`);
    ...
    socket.on("add_group_members", async (data, callback) => {
      console.log(
        `[ADD_GROUP_MEMBERS] Attempting to add members: userId=${socket.userId
        }, data=${JSON.stringify(data)}`
      );
    ...
    socket.on("update_group", async (data, callback) => {
      console.log(
        `[UPDATE_GROUP] Attempting to update: userId=${socket.userId
        }, data=${JSON.stringify(data)}`
      );
      try {
    ...
    socket.on("remove_group_member", async (data, callback) => {
      console.log(
        `[REMOVE_GROUP_MEMBER] Attempting to remove member: userId=${socket.userId
        }, data=${JSON.stringify(data)}`
      );
      try {
    ...
    socket.on("send_text_message", async (data, callback) => {
      ...
      console.log(
        `[SEND_TEXT_MESSAGE] Attempting to send message: socketId=${socket.id
        }, userId="${socket.userId
        }" (type: ${typeof socket.userId}), data=${JSON.stringify(
          data,
          null,
          2
        )}, timestamp=${timestamp}`
      ...
      try {
        ...
        console.log(
          `[SEND_TEXT_MESSAGE_DEBUG] Step 1: Validating socket.userId: "${senderIdStr}", timestamp=${timestamp}`
        );
        if (!senderIdStr) {
          console.error(
            `[SEND_TEXT_MESSAGE_ERROR] socket.userId is null or undefined: socketId=${socket.id}, timestamp=${timestamp}`
          );
          return callback({
            success: false,
            message:
              "Authentication failed: user ID is missing. Please join groups first.",
          });
        }
        if (typeof senderIdStr !== "string") {
          console.error(
            `[SEND_TEXT_MESSAGE_ERROR] socket.userId is not a string: value="${senderIdStr}", type=${typeof senderIdStr}, socketId=${socket.id
            }, timestamp=${timestamp}`
          );
          return callback({
            success: false,
        ...
        console.log(
          `[SEND_TEXT_MESSAGE_DEBUG] Step 2: Checking onlineUsers: has=${onlineUsers.has(
            senderIdStr
          )}, timestamp=${timestamp}`
        );
        if (!onlineUsers.has(senderIdStr)) {
          console.error(
            `[SEND_TEXT_MESSAGE_ERROR] User not in onlineUsers: senderId=${senderIdStr}, socketId=${socket.id}, timestamp=${timestamp}`
          );
          return callback({
            success: false,
        ...
        console.log(
          `[SEND_TEXT_MESSAGE_DEBUG] Step 3: Validating ObjectId: isValid=${isValidObjectId(
            senderIdStr
          )}, timestamp=${timestamp}`
        );
        ...
        console.log(
          `[SEND_TEXT_MESSAGE_DEBUG] Casted senderId: ${senderId.toString()}, timestamp=${timestamp}`
        );

        // Step 4: Check if User exists and get displayName
        console.log(
          `[SEND_TEXT_MESSAGE_DEBUG] Step 4: Querying User by _id: ${senderId.toString()}, timestamp=${timestamp}`
        );
        const user = await User.findById(senderId).select("displayName phone");
        console.log(
          `[SEND_TEXT_MESSAGE_DEBUG] User query result: ${user ? "Found" : "Not Found"
          }, user=${JSON.stringify(
        ...
        if (!user) {
          console.error(
            `[SEND_TEXT_MESSAGE_ERROR] User not found in database: senderId=${senderId}, socketId=${socket.id}, timestamp=${timestamp}`
          );
          return callback({
            success: false,
        ...
        console.log(
          `[SEND_TEXT_MESSAGE_DEBUG] Step 5: Validating groupId: "${groupId}", isValid=${isValidObjectId(
            groupId
          )}, timestamp=${timestamp}`
        );
        if (!groupId || !isValidObjectId(groupId)) {
          console.error(
            `[SEND_TEXT_MESSAGE_ERROR] Invalid or missing groupId: "${groupId}", timestamp=${timestamp}`
          );
          return callback({
            success: false,
        ...
        console.log(
          `[SEND_TEXT_MESSAGE_DEBUG] Casted groupId: ${castGroupId.toString()}, timestamp=${timestamp}`
        );

        // Step 6: Validate content
        console.log(
          `[SEND_TEXT_MESSAGE_DEBUG] Step 6: Validating content: length=${content ? content.trim().length : 0
          }, timestamp=${timestamp}`
        );
        if (!content || content.trim() === "") {
          console.error(
            `[SEND_TEXT_MESSAGE_ERROR] Empty content: "${content}", timestamp=${timestamp}`
          );
          return callback({
            success: false,
        ...
        console.log(
          `[SEND_TEXT_MESSAGE_DEBUG] Step 7: Querying Group by _id: ${castGroupId.toString()}, timestamp=${timestamp}`
        );
        const group = await Group.findById(castGroupId);
        console.log(
          `[SEND_TEXT_MESSAGE_DEBUG] Group query result: ${group ? "Found" : "Not Found"
        ...
        if (!group) {
          console.error(
            `[SEND_TEXT_MESSAGE_ERROR] Group not found: groupId=${castGroupId}, timestamp=${timestamp}`
          );
          return callback({
            success: false,
        ...
        console.log(
          `[SEND_TEXT_MESSAGE_DEBUG] Checking membership: members=${group.members
            .map((id) => id.toString())
            .join(", ")}, timestamp=${timestamp}`
        );
        const isMember = group.members.some((id) => id.equals(senderId));
        console.log(
          `[SEND_TEXT_MESSAGE_DEBUG] Is member: ${isMember}, timestamp=${timestamp}`
        );
        if (!isMember) {
          console.error(
            `[SEND_TEXT_MESSAGE_ERROR] Not authorized: senderId=${senderId}, groupId=${castGroupId}, timestamp=${timestamp}`
          );
          return callback({
            success: false,
        ...
        console.log(
          `[SEND_TEXT_MESSAGE_DEBUG] Step 8: Creating Chat document, timestamp=${timestamp}`
        );
        const chat = new Chat({
          senderId,
        ...
        await chat.save();
        console.log(
          `[SEND_TEXT_MESSAGE_DEBUG] Message saved: messageId=${chat._id
          }, groupId=${castGroupId}, senderId=${senderId}, rawChat=${JSON.stringify(
            chat.toObject(),
            null,
            2
          )}, timestamp=${timestamp}`
        ...
        console.log(
          `[SEND_TEXT_MESSAGE_DEBUG] Step 10: Scheduling status update to delivered, timestamp=${timestamp}`
        );
        setTimeout(async () => {
          try {
            const updateTimestamp = new Date().toISOString();
            console.log(
              `[SEND_TEXT_MESSAGE_DEBUG] Updating status for messageId=${chat._id}, timestamp=${updateTimestamp}`
            );
            ...
            if (updatedChat) {
              ...
              console.log(
                `[SEND_TEXT_MESSAGE_DEBUG] Updated status to delivered: messageId=${chat._id}, timestamp=${updateTimestamp}`
              );
            } else {
              console.warn(
                `[SEND_TEXT_MESSAGE_WARN] Failed to update status for messageId=${chat._id}, timestamp=${updateTimestamp}`
              );
            }
          } catch (error) {
            console.error(
              `[SEND_TEXT_MESSAGE_ERROR] Failed to update status: messageId=${chat._id
        ...
        console.log(
          `[SEND_TEXT_MESSAGE_DEBUG] Step 11: Sending success response, timestamp=${timestamp}`
        );
        ...
        console.error(
          `[SEND_TEXT_MESSAGE_ERROR] Failed: socketId=${socket.id}, userId="${socket.userId
          }", error=${error.message}, stack=${error.stack
          }, timestamp=${new Date().toISOString()}`
        );
        callback({
    ...
    socket.on(
      ...
      async (
        ...
        const timestamp = moment()
          ...
          .format("DD/MM/YYYY, hh:mm:ss a");
        console.log(
          `[SEND_VOICE_MESSAGE] Attempting to send voice message: userId=${socket.userId
          }, data=${JSON.stringify(
            { senderId, groupId, content, duration, fileType, fileName },
            null,
            2
        ...
        try {
          ...
          console.log(
            `[SEND_VOICE_MESSAGE_DEBUG] Step 3: Querying User by _id: ${castSenderId.toString()}, timestamp=${timestamp}`
          );
          const user = await User.findById(castSenderId).select(
            "displayName phone"
          );
          console.log(
            `[SEND_VOICE_MESSAGE_DEBUG] User query result: ${user ? "Found" : "Not Found"
            }, user=${JSON.stringify(
              user ? user.toObject() : null,
              null,
              2
          ...
          if (!user) {
            console.error(
              `❌ [SEND_VOICE_MESSAGE_ERROR] User not found in database: senderId=${castSenderId}, timestamp=${timestamp}`
            );
            socket.emit("voice_error", {
          ...
          const senderDisplayName = user.displayName;
          console.log(
            `[SEND_VOICE_MESSAGE_DEBUG] Found user: senderId=${castSenderId}, displayName=${senderDisplayName}, timestamp=${timestamp}`
          );
    ...
    socket.on("mark_message_read", async (data, callback) => {
      console.log(
        `[MARK_MESSAGE_READ] Attempting to mark message read: userId=${socket.userId
        }, data=${JSON.stringify(data)}`
      );
    ...
    socket.on("get_group_messages", async (data, callback) => {
      console.log(
        `[GET_GROUP_MESSAGES] Fetching messages: userId=${socket.userId
        }, data=${JSON.stringify(data)}`
      );
    ...
    socket.on("delete_message", async (data, callback) => {
      console.log(
        `[DELETE_MESSAGE] Attempting to delete message: userId=${socket.userId
        }, data=${JSON.stringify(data)}`
      );
    ...
    socket.on("send_media", async (payload, callback) => {
      ...
      try {
        ...
        console.log(
          `[SEND_MEDIA_DEBUG] Step 5: Querying User by _id: ${senderId.toString()}, timestamp=${timestamp}`
        );
        const user = await User.findById(senderId).select("displayName phone");
        console.log(
          `[SEND_MEDIA_DEBUG] User query result: ${user ? "Found" : "Not Found"
          }, user=${JSON.stringify(
            user ? user.toObject() : null,
            null,
            2
        ...
        if (!user) {
          console.error(
            `[SEND_MEDIA_ERROR] User not found in database: senderId=${senderId}, timestamp=${timestamp}`
          );
          return ack("User not found - please verify your account.");
        }
        const senderDisplayName = user.displayName;
        console.log(
          `[SEND_MEDIA_DEBUG] Found user: senderId=${senderId}, displayName=${senderDisplayName}, timestamp=${timestamp}`
        );
