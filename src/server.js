...
const groupIo = initGroupSocket(server);

chatIo.on("connection", (socket) => {
  console.log(
    `⚡ New chat client connected: ${socket.id} at`,
    new Date().toLocaleString("en-PK", { timeZone: "Asia/Karachi" })
  );
  socket.on("disconnect", () => {
    console.log(
      `❌ Chat client disconnected: ${socket.id} at`,
      new Date().toLocaleString("en-PK", { timeZone: "Asia/Karachi" })
    );
  });
});
...
videoIo.on("connection", (socket) => {
  console.log(
    `🎥 New video client connected: ${socket.id} at`,
    new Date().toLocaleString("en-PK", { timeZone: "Asia/Karachi" })
  );
  socket.on("disconnect", () => {
    console.log(
      `❌ Video client disconnected: ${socket.id} at`,
      new Date().toLocaleString("en-PK", { timeZone: "Asia/Karachi" })
    );
  });
});
...
audioIo.on("connection", (socket) => {
  console.log(
    `🎙️ New audio client connected: ${socket.id} at`,
    new Date().toLocaleString("en-PK", { timeZone: "Asia/Karachi" })
  );
  socket.on("disconnect", () => {
    console.log(
      `❌ Audio client disconnected: ${socket.id} at`,
      new Date().toLocaleString("en-PK", { timeZone: "Asia/Karachi" })
    );
  });
});
...
server.listen(PORT, () => {
  console.log(
    `🚀 Server running at http://localhost:${PORT} at`,
    new Date().toLocaleString("en-PK", { timeZone: "Asia/Karachi" })
  );
});
