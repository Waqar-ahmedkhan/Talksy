...
// Ensure media directions are sendrecv where we have local tracks.
  ...
  void _setupSocketListeners(String userId) {
    ...
    videoSocket!.onConnect((_) {
      ...
      videoSocket!.emit('join', {'userId': userId});
    });
    ...
    videoSocket!.on('incoming_call', (data) async {
      ...
      if (data != null && data is Map<String, dynamic>) {
        ...
        final callerName = data['callerName'] as String? ?? callerId;
        final callerPhone = data['callerPhone'] as String? ?? callerId;
        ...
        try {
          ...
          currentCallerUserId.value = callerId; // remote party
          currentCalleeUserId.value = selfId; // this device user
          _v('📞 IDs set on incoming before PC create: caller=$callerId, self=$selfId');
        } catch (e) {
        ...
        _showIncomingCallScreen(callerId: callerId, callType: callType);
      }
    });
    ...
    try {
      ...
      if (isCaller) {
        ...
        stream = await webrtc.navigator.mediaDevices.getUserMedia({
          ...
          'video': isEmulator
              ? {
                  // Simplified constraints for emulator
                  'facingMode': 'user',
                  'width': {'ideal': 640},
                  'height': {'ideal': 480},
                  'frameRate': {'ideal': 15},
              ...
              : {
                  // Full constraints for real devices
                  'facingMode': 'user',
                  'width': {'min': 320, 'ideal': 640, 'max': 1280},
                  'height': {'min': 240, 'ideal': 480, 'max': 720},
      ...
      peerConnection!.onIceCandidate = (webrtc.RTCIceCandidate? candidate) {
        if (candidate != null && callState.value != 'ended') {
          ...
          if (candidateStr.contains('typ host'))
            ...
            type = 'TURN (Relay Server)';

          _v('📡 ICE CANDIDATE GATHERED: $type');
          _v('   - Candidate: ${candidate.candidate}');

          if (type == 'TURN (Relay Server)') {
            _v('✅ PROOF: TURN Server is working and gathering relay candidates.');
          }

          String? targetUserId;
          if (callState.value == 'calling') {
            targetUserId = currentCalleeUserId.value;
            debugPrint('📞 Sending ICE candidate to callee: $targetUserId');
            ...
            debugPrint('📞 Current callee ID: ${currentCalleeUserId.value}');
              ...
              callState.value == 'incoming') {
            // For incoming calls or connected state, send to the other party
            targetUserId =
                currentCalleeUserId.value ?? currentCallerUserId.value;
            debugPrint('📞 Sending ICE candidate to peer: $targetUserId');
            debugPrint('📞 Current caller ID: ${currentCallerUserId.value}');
            debugPrint('📞 Current callee ID: ${currentCalleeUserId.value}');
          }
      ...
      peerConnection!.onIceConnectionState =
        ...
        _v('🛡️ ICE CONNECTION STATE: $state');

        if (state == webrtc.RTCIceConnectionState.RTCIceConnectionStateFailed) {
          _v('❌❌❌ PROOF OF INFRASTRUCTURE ISSUE ❌❌❌');
          _v('   The connection failed because no media path could be established.');
          _v('   Possible reasons:');
          _v('   1. Both users are on different networks and TURN server failed/missing.');
          _v('   2. Firewall/Network is blocking UDP ports.');
          _v('   3. Backend did not deliver ICE candidates to the other peer.');
          _v('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
        }
      };
  ...
  Future<void> initiateVideoCall({
    ...
    try {
      debugPrint('📞 Initiating video call from $callerId to $calleeId');
      ...
      currentCallerUserId.value = callerId;
      currentCalleeUserId.value = calleeId;
      ...
      if (calleeFcmToken != null && calleeFcmToken.isNotEmpty) {
        try {
          final callerPhone = await AuthStorage.getPhoneNumber() ?? callerId;
      ...
      try {
        // STANDARD FIX: Force VP8 Software Codec
        // We rely on VP8 to avoid hardware encoder failures (0x0 video)
        ...
        offer = webrtc.RTCSessionDescription(mungedSdp, offer.type);
      } catch (e) {
        debugPrint('⚠️ Failed to prepare offer SDP: $e');
      }
      await peerConnection!.setLocalDescription(offer);
      debugPrint('📞 Local description set successfully');

      final callData = {
        'callerId': callerId, // MongoDB user ID - server uses this for matching
        'calleeId': calleeId, // Use actual callee ID from parameter
        'offer': {
          'sdp': offer.sdp,
          'type': offer.type,
        },
      ...
      _showVideoCallingScreen(
        contactName: calleeName ?? calleeId,
        contactPhone: calleeId,
        isIncoming: false,
      );
  ...
  Future<void> acceptCall({
    ...
    try {
      ...
      // Prefer VP8 and ensure sendrecv for proper bidirectional communication
      try {
        ...
        answer = webrtc.RTCSessionDescription(munged, answer.type);
      } catch (e) {
        debugPrint('⚠️ Failed to munge SDP: $e');
      }
      await peerConnection!.setLocalDescription(answer);
      debugPrint('📞 Answer created');
      debugPrint('📞 Answer SDP: ${answer.sdp?.substring(0, 200)}...');

      // Log if video SSRC is present
      if (answer.sdp!.contains('a=ssrc') && answer.sdp!.contains('m=video')) {
      ...
      debugPrint('📤📤📤 ========================================');
      debugPrint('📤 Caller ID (will receive call_accepted): $callerId');
      debugPrint('📤 Callee ID (this device): $calleeId');
      ...
      debugPrint(
          '⏳ Waiting for caller ($callerId) to receive call_accepted event...');
      debugPrint('');
  ...
  Future<void> rejectCall({
    ...
    try {
      debugPrint('📞 Rejecting video call from $callerId');
      ...
      final rejectData = {
        'callerId': callerId, // Use actual caller ID from parameter
        'calleeId': calleeId, // Use actual callee ID from parameter
      };
  ...
  void _showIncomingCallScreen(
      {required String callerId, required String callType}) {
    debugPrint('📞 Showing incoming video call screen for: $callerId');
...
String _preferVp8(String sdp) {
  try {
    ...
    // Build new order: VP8 first if present, then the rest preserving order
    final vp8Pts = [...?codecPts['VP8']];
    if (vp8Pts.isEmpty) return sdp; // nothing to prefer
    ...
    for (final pt in payloads) {
      if (seen.add(pt)) reordered.add(pt);
    }
